...
public class ATM {
	...
	@Test
	public void testShowBalanceBeforeWithdrawal() {
		ATM atm = new ATM();

		assertNotNull(atm.screen);	// Asserts that an object isn't null.
		//assertNotNull(java.lang.String message, java.lang.Object object)
		assertEquals(1, BALANCE_INQUIRY);	//Asserts that two longs are equal.
	...
	public void testWithdrawal() {
		ATM atm = new ATM();
		Withdrawal withdrawal = new Withdrawal(12345, new Screen(), new BankDatabase(), new Keypad(),
				new CashDispenser());

		assertNotNull("cashDispenser is Null", atm.cashDispenser);	
		//assertNotNull(java.lang.String message, java.lang.Object object)
		assertTrue("Larger", 500.0 < withdrawal.getBankDatabase().getAvailableBalance(withdrawal.getAccountNumber()));	
		//assertTrue(java.lang.String message, boolean condition)
	}
	
	@Test
	public void testShowBalanceAfterWithdrawal() {
		Withdrawal withdrawal = new Withdrawal(12345, new Screen(), new BankDatabase(), new Keypad(),
				new CashDispenser());

		withdrawal.getBankDatabase().debit(12345, 100.0);
		assertEquals((int)900.0, (int)withdrawal.getBankDatabase().getAvailableBalance(12345));
		assertTrue(withdrawal.getBankDatabase().getAvailableBalance(12345) < 1000.0);
	}
}
